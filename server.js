...
// Initialize all CDP connections (multi-instance)
async function initAllCDP() {
    console.log('🔍 Discovering all Antigravity instances...');
    const instances = await discoverAllCDP();

    if (instances.length === 0) {
        throw new Error('No Antigravity instances found on ports ' + PORTS.join(', '));
    }

    for (const inst of instances) {
        if (cdpInstances.has(inst.port)) continue; // Already connected

        try {
            console.log(`🔌 Connecting to ${inst.title} on port ${inst.port}...`);
            const cdp = await connectCDP(inst.url);
            cdpInstances.set(inst.port, {
                cdp,
                title: inst.title,
            ...
            console.log(`✅ Connected to ${inst.title} (port ${inst.port}, ${cdp.contexts.length} contexts)`);
        } catch (e) {
            console.warn(`⚠️  Failed to connect to port ${inst.port}: ${e.message}`);
        }
    }

    // Auto-select first instance if none active
    if (!activePort || !cdpInstances.has(activePort)) {
        const firstPort = cdpInstances.keys().next().value;
        if (firstPort !== undefined) {
            activePort = firstPort;
            console.log(`📌 Active workspace: ${cdpInstances.get(activePort).title} (port ${activePort})`);
        }
    }
}

// Background polling (multi-instance aware)
async function startPolling(wss) {
    let lastErrorLog = 0;
    let lastDiscovery = 0;
    const DISCOVERY_INTERVAL = 5000; // Re-scan for new instances every 5s

    const poll = async () => {
        const now = Date.now();

        // Periodically re-discover instances (find new ones, clean up closed ones)
        if (now - lastDiscovery > DISCOVERY_INTERVAL) {
            lastDiscovery = now;

            try {
                const liveInstances = await discoverAllCDP();
                const livePorts = new Set(liveInstances.map(i => i.port));

                // Remove closed instances
                for (const [port, inst] of cdpInstances) {
                    if (!livePorts.has(port) || (inst.cdp.ws && inst.cdp.ws.readyState !== WebSocket.OPEN)) {
                        console.log(`🔌 Instance on port ${port} (${inst.title}) disconnected.`);
                        try { inst.cdp.ws.close(); } catch (e) { }
                        cdpInstances.delete(port);

                        // If active port was removed, switch to another
                        if (activePort === port) {
                            const nextPort = cdpInstances.keys().next().value;
                            activePort = nextPort !== undefined ? nextPort : null;
                            if (activePort) {
                                console.log(`📌 Auto-switched to: ${cdpInstances.get(activePort).title} (port ${activePort})`);
                            }
                            // Notify clients of workspace change
                            wss.clients.forEach(client => {
                                if (client.readyState === WebSocket.OPEN) {
                                    client.send(JSON.stringify({ type: 'workspace_changed' }));
                                }
                            });
                        }
                    }
                ...
                for (const inst of liveInstances) {
                    if (!cdpInstances.has(inst.port)) {
                        try {
                            ...
                            console.log(`✅ New workspace discovered: ${inst.title} (port ${inst.port})`);

                            // Auto-select if no active
                            if (!activePort) {
                                activePort = inst.port;
                                console.log(`📌 Auto-selected: ${inst.title}`);
                            }

                            // Notify clients of new workspace
                            wss.clients.forEach(client => {
                                if (client.readyState === WebSocket.OPEN) {
                                    client.send(JSON.stringify({ type: 'workspaces_updated' }));
                                }
                            });
                        } catch (e) {
        ...
        // No active instance - wait for one
        if (!activePort || !cdpInstances.has(activePort)) {
            setTimeout(poll, 2000);
            return;
        }

        // Capture snapshot for active instance only
        const activeInst = cdpInstances.get(activePort);
        if (!activeInst || !activeInst.cdp || activeInst.cdp.ws.readyState !== WebSocket.OPEN) {
            setTimeout(poll, 2000);
            return;
        }

        try {
            const snapshot = await captureSnapshot(activeInst.cdp);
            if (snapshot && !snapshot.error) {
                const hash = hashString(snapshot.html);

                // Only update if content changed
                if (hash !== activeInst.lastSnapshotHash) {
                    activeInst.lastSnapshot = snapshot;
                    activeInst.lastSnapshotHash = hash;
                    ...
                    console.log(`📸 [${activeInst.title}] Snapshot updated (hash: ${hash})`);
                }
            } else {
                ...
                if (!lastErrorLog || now - lastErrorLog > 10000) {
                    ...
                    console.warn(`⚠️  [${activeInst.title}] Snapshot issue: ${errorMsg}`);
            ...
            console.error(`Poll error [${activeInst.title}]:`, err.message);
        }
...
async function createServer() {
    ...
    app.use((req, res, next) => {
        ...
        if (req.xhr || req.headers.accept?.includes('json') || req.path.startsWith('/snapshot') || req.path.startsWith('/send') || req.path.startsWith('/upload-media')) {
    ...
    app.get('/health', (req, res) => {
        const cdp = getActiveCDP();
        res.json({
            status: 'ok',
            cdpConnected: cdp?.ws?.readyState === 1,
            activePort: activePort,
            instanceCount: cdpInstances.size,
    ...
    app.post('/send', async (req, res) => {
        ...
        const result = await injectMessage(cdp, message);
    ...
    app.post('/upload-media', upload.array('files', 10), (req, res) => {
        ...
        console.log(`📎 Uploaded ${req.files.length} file(s):`, paths.map(p => p.split('/').pop()));
    ...
    app.get('/workspaces', (req, res) => {
        ...
        for (const [port, inst] of cdpInstances) {
            workspaces.push({
                ...
                connected: inst.cdp?.ws?.readyState === WebSocket.OPEN,
                hasSnapshot: !!inst.lastSnapshot
            });
        }
        res.json({
            activePort,
    ...
    app.post('/switch-workspace', (req, res) => {
        const { port } = req.body;
        const targetPort = parseInt(port);

        if (!targetPort || !cdpInstances.has(targetPort)) {
            return res.status(400).json({
                error: 'Invalid workspace port',
                available: Array.from(cdpInstances.keys())
            });
        }

        const inst = cdpInstances.get(targetPort);
        activePort = targetPort;
        console.log(`📌 Switched to workspace: ${inst.title} (port ${targetPort})`);
...
async function main() {
    try {
        ...
        console.log('💡 The server will keep scanning for instances in the background.');
    }

    try {
        const { server, wss, app, hasSSL } = await createServer();

        // Start background polling (handles multi-instance discovery + reconnections)
        startPolling(wss);
        ...
        const gracefulShutdown = (signal) => {
            ...
            for (const [port, inst] of cdpInstances) {
                try {
                    if (inst.cdp?.ws) inst.cdp.ws.close();
                    console.log(`   CDP connection closed (port ${port})`);
                } catch (e) { }
            }
